...
# Is instruction valid? #IIADDQ should be a valid instruction
...
# Does fetched instruction require a regid byte? #Yes it does just like IOP required a byte IIADDQ requires one
bool need_regids =
	f_icode in { IRRMOVQ, IOPQ, IPUSHQ, IPOPQ, 
		     IIRMOVQ, IRMMOVQ, IMRMOVQ, IIADDQ };

# Does fetched instruction require a constant word? #Yes, because the immediate value has to be stored
bool need_valC =
...
## What register should be used as the A source? # it should be empty, we're adding to rB
word d_srcA = [
...
## What register should be used as the B source? #IIADDQ adds a value to register rB, so rB
word d_srcB = [
...
## What register should be used as the E destination? #rB, because we want to increase its value
word d_dstE = [
...
## What register should be used as the M destination? # we do not need to use data from memory
word d_dstM = [
	D_icode in { IMRMOVQ, IPOPQ } : D_rA;
	1 : RNONE;  # Don't write any register
];

## What should be the A value? # nothing, because we didn't use rA
...
## Select input A to ALU # immediate value added to register
word aluA = [
...
## Select input B to ALU #register
word aluB = [
...
## Should the condition codes be updated? ##yes, because our register might encounter a zero flag, overflow flag...etc
